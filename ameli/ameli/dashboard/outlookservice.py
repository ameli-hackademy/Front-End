...
def get_my_messages(access_token):
  ...
  days = 10
  testdate = (date.today() - timedelta(days=days)).strftime("%Y-%m-%d")
  query_parameters = {'$filter':'ReceivedDateTime ge {}'.format(testdate),
...
def get_my_rule(access_token):
    get_rule_url= graph_endpoint.format('/me/mailFolders/inbox/messageRules')

    r = make_api_call('GET',get_rule_url,access_token)
    if (r.status_code == requests.codes.ok):
        return r.json()
    else:
